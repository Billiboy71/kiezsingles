<?php
// ============================================================================
// File: C:\laragon\www\kiezsingles\resources\views\admin\security\allowlist\device\index.blade.php
// Purpose: Admin Security - Manage device allowlist entries for autoban exclusions
...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.security.allowlist.device.store') }}" class="ks-card mb-4 grid grid-cols-1 md:grid-cols-4 gap-3">
        @csrf
        <div class="md:col-span-4 flex items-start justify-between gap-3">
            <h3>Device-Allowlist-Eintrag anlegen</h3>
        </div>
        <div><label>Device / Pattern</label><input class="w-full" type="text" name="value" required></div>
    ...
    <div class="ks-card">
        <div class="flex items-center justify-end mb-2">
            <form method="GET" action="{{ route('admin.security.allowlist.device.index') }}" class="flex items-center gap-2 text-xs">
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[980px] text-sm">
                ...
                @forelse($allowlistEntries as $entry)
                    <tr>
                        ...
                        <td class="px-3 py-2 align-top max-w-[360px] break-words">
                            <form method="POST" action="{{ route('admin.security.allowlist.device.update', $entry->id) }}" class="flex flex-col gap-2">
                                @csrf
                                @method('PATCH')
                        ...
                        <td class="px-3 py-2 align-top whitespace-nowrap">
                            <form method="POST" action="{{ route('admin.security.allowlist.device.destroy', $entry->id) }}">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr><td colspan="7" class="px-3 py-3">Keine Device-Allowlist-Einträge vorhanden.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
