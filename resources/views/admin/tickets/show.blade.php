...
@section('content')
    ...
    <div id="ks_ticket_page" data-ticket-id="{{ $ticketId }}" data-draft-save-url="{{ $draftSaveUrl }}">

        <div class="ks-top">
            <div class="ks-row">
                <h1 class="ks-h1">Ticket {{ $ticketId }}</h1>
            </div>

            <div style="flex:1; display:flex; justify-content:center;">
                <span id="js-status-badge" class="ks-badge {{ $statusClass }}">{{ $statusLabel }}</span>
            </div>

            <div class="ks-row">
                <a class="ks-btn" href="{{ route('admin.tickets.index') }}" style="text-decoration:none; color:#111;">Zurück zur Liste</a>
            </div>
        </div>

        @if($notice)
            <div class="ks-notice">{{ (string) $notice }}</div>
        @endif

        <div class="ks-card">

            <div class="ks-kvline" style="margin:0 0 10px 0;">
                <div class="ks-kvbox"><b>Typ</b><span>{{ $typeLabel }}</span></div>

                <div class="ks-kvbox">
                    <b>Kategorie</b>
                    <span>
                        <span id="js-head-category-badge" class="ks-badge {{ $categoryClass }}">{{ $category !== '' ? $categoryLabel : '-' }}</span>
                    </span>
                </div>
                ...
                <div class="ks-kvbox">
                    <b>Priorität</b>
                    <span>
                        <span id="js-head-priority-badge" class="ks-badge {{ $priorityClass }}">{{ $priorityLabel !== '' ? $priorityLabel : '-' }}</span>
                    </span>
                </div>

                <div class="ks-kvbox creator"><b>Ersteller</b><span>{{ $creatorDisplay }}</span></div>
                <div class="ks-kvbox reported"><b>Gemeldet</b><span>{{ $reportedDisplay }}</span></div>
                <div class="ks-kvbox assigned"><b>Zugewiesen</b><span id="js-head-assigned">{{ $assignedAdminDisplay }}</span></div>
            </div>

            <div class="ks-row" style="margin:0 0 0 0;">
                <div class="ks-kvbox ctime"><b>Erstellt</b><span>{{ $createdAt }}</span></div>
                <div class="ks-kvbox closedtime"><b>Geschlossen</b><span>{!! $closedAt !== '' ? e($closedAt) : '<span style="color:#666;">-</span>' !!}</span></div>
            </div>
        ...
        <div class="ks-card">
            ...
            <pre>{!! $messageText !== '' ? e($messageText) : '<span style="color:#666;">(ohne)</span>' !!}</pre>
        </div>
        ...
        @if($isReport)
            ...
            <div class="ks-card">
                ...
                <div class="ks-row" style="margin:0 0 12px 0;">
                    ...
                    <form method="POST" action="{{ route('admin.tickets.moderate.warn', $ticketId) }}" style="margin:0;">
                        @csrf
                    ...
                    <form method="POST" action="{{ route('admin.tickets.moderate.tempBan', $ticketId) }}" style="margin:0;">
                        @csrf
                    ...
                    <form method="POST" action="{{ route('admin.tickets.moderate.permBan', $ticketId) }}" style="margin:0;">
                        @csrf
                        <input class="ks-input" type="text" name="note" placeholder="Notiz (optional)" style="min-width:240px;">
                        <button type="submit" class="ks-btn">Dauerhaft</button>
                    </form>

                    <form method="POST" action="{{ route('admin.tickets.moderate.unfounded', $ticketId) }}" style="margin:0;">
                        @csrf
                        <input class="ks-input" type="text" name="note" placeholder="Notiz (optional)" style="min-width:240px;">
        ...
        <div class="ks-card">
            <form id="js-reply-form" method="POST" action="{{ route('admin.tickets.reply', $ticketId) }}">
                ...
                <textarea id="js-reply-message" class="ks-textarea" name="reply_message" placeholder="Antwort an den Nutzer...">{{ $draftReplyText }}</textarea>

                <div class="ks-label" style="margin:12px 0 6px 0;">Interne Admin-Notiz (nicht sichtbar für den Nutzer)</div>
                <textarea id="js-internal-note" class="ks-textarea" name="internal_note" placeholder="Interne Notiz...">{{ $draftInternalText }}</textarea>
