...
function Invoke-KsAuditCheck_SecurityAbuseProtection {
    ...
    function AddSub([System.Collections.Generic.List[object]]$arr, [string]$n, [string]$s, [string]$sum, [int]$ms) { $arr.Add([pscustomobject]@{ name = $n; status = $s; summary = $sum; duration_ms = $ms }) | Out-Null }
    ...
    if (-not $probe) { AddSub $sub "Security Login Rate Limit" "SKIP" "SecurityProbe=false (active probe disabled)." 0 }
    else {
        $sess = $null; try { $sess = New-Object Microsoft.PowerShell.Commands.WebRequestSession } catch { $sess = $null }
        if ($null -eq $sess) { AddSub $sub "Security Login Rate Limit" "FAIL" "Cannot initialize WebRequestSession." 0 }
        else {
            $g = Req "$baseUrl/login" "GET" $sess $null 3
            if (-not $g.ok) { $sa.Stop(); AddSub $sub "Security Login Rate Limit" "FAIL" ("GET /login failed: " + $g.error) ([int]$sa.ElapsedMilliseconds) }
            else {
                ...
                if ($tok -eq "") { $sa.Stop(); AddSub $sub "Security Login Rate Limit" "FAIL" "GET /login did not expose _token." ([int]$sa.ElapsedMilliseconds) }
                else {
                    ...
                    if ($hit419) { AddSub $sub "Security Login Rate Limit" "FAIL" "Probe invalid: POST /login returned 419 (csrf_419)." ([int]$sa.ElapsedMilliseconds) }
                    elseif ($found -and $expect429 -and ($sig -notmatch '^429')) { AddSub $sub "Security Login Rate Limit" "WARN" ("Lockout signal detected, but no 429 while SecurityExpect429=true ($sig).") ([int]$sa.ElapsedMilliseconds) }
                    elseif ($found) { AddSub $sub "Security Login Rate Limit" "OK" ("Lockout after <= $attempts attempts ($sig).") ([int]$sa.ElapsedMilliseconds) }
                    elseif ($sig -like "request_error:*") { AddSub $sub "Security Login Rate Limit" "FAIL" ("Probe failed: $sig") ([int]$sa.ElapsedMilliseconds) }
                    else { AddSub $sub "Security Login Rate Limit" "FAIL" ("No throttle/lockout signal after $attempts failed logins.") ([int]$sa.ElapsedMilliseconds) }
                }
            }
        }
    }
    ...
    if (-not $probe) { AddSub $sub "Security IP Ban" "SKIP" "SecurityProbe=false (active probe disabled)." 0 }
    elseif (-not $checkIpBan) { AddSub $sub "Security IP Ban" "SKIP" "SecurityCheckIpBan=false." 0 }
    else {
        ...
        $sb.Stop()
        if ($ok) { AddSub $sub "Security IP Ban" "OK" ("Enforcement visible ($sig).") ([int]$sb.ElapsedMilliseconds) }
        elseif ($ipBanExists -and $ipBanCount -gt 0) { AddSub $sub "Security IP Ban" "WARN" ("Ban records exist ($ipBanCount), but enforcement not visible.") ([int]$sb.ElapsedMilliseconds) }
        elseif ($e.ok -and -not $e.exists) { AddSub $sub "Security IP Ban" "SKIP" "security_ip_bans table not present; mechanism appears disabled." ([int]$sb.ElapsedMilliseconds) }
        elseif ($err -ne "") { AddSub $sub "Security IP Ban" "WARN" ("Probe request issue: $err") ([int]$sb.ElapsedMilliseconds) }
        else { AddSub $sub "Security IP Ban" "WARN" "Not enforced or not configured." ([int]$sb.ElapsedMilliseconds) }
    }
    ...
    if ($null -eq $rr) { $sc.Stop(); AddSub $sub "Security Middleware" "WARN" "route:list -vv could not be executed." ([int]$sc.ElapsedMilliseconds) }
    else {
        $o = ""; try { $o = ("" + $rr.StdOut) } catch { $o = "" }
        if ([int]$rr.ExitCode -ne 0 -and $o.Trim() -eq "") { $sc.Stop(); AddSub $sub "Security Middleware" "WARN" ("route:list -vv failed (exit $($rr.ExitCode)).") ([int]$sc.ElapsedMilliseconds) }
        else {
            ...
            if (@($crit).Count -eq 0) { $sc.Stop(); AddSub $sub "Security Middleware" "SKIP" "No critical admin routes found for step-up validation." ([int]$sc.ElapsedMilliseconds) }
            else {
                ...
                $sc.Stop()
                if ($covered -eq @($crit).Count) { AddSub $sub "Security Middleware" "OK" ("Step-up/fresh-session middleware present on $covered critical route(s).") ([int]$sc.ElapsedMilliseconds) }
                elseif ($covered -gt 0) { AddSub $sub "Security Middleware" "WARN" ("Step-up present on $covered/$(@($crit).Count) critical route(s); missing on: " + ($miss -join ", ")) ([int]$sc.ElapsedMilliseconds) }
                else { AddSub $sub "Security Middleware" "WARN" ("No step-up middleware found on $(@($crit).Count) critical route(s).") ([int]$sc.ElapsedMilliseconds) }
            }
        }
    }
    ...
    $sd.Stop(); if ($failed) { AddSub $sub "Security Tables" "FAIL" (($tDet.ToArray()) -join "; ") ([int]$sd.ElapsedMilliseconds) } else { AddSub $sub "Security Tables" "OK" (($tDet.ToArray()) -join "; ") ([int]$sd.ElapsedMilliseconds) }

    # E
    $se = [System.Diagnostics.Stopwatch]::StartNew()
    if (-not $probe) { AddSub $sub "Security Registration Abuse" "SKIP" "SecurityProbe=false (active probe disabled)." 0 }
    elseif (-not $checkReg) { AddSub $sub "Security Registration Abuse" "SKIP" "SecurityCheckRegister=false." 0 }
    else {
        ...
        if (-not $hasReg) { $se.Stop(); AddSub $sub "Security Registration Abuse" "SKIP" "/register route not present." ([int]$se.ElapsedMilliseconds) }
        else {
            $sess = $null; try { $sess = New-Object Microsoft.PowerShell.Commands.WebRequestSession } catch { $sess = $null }
            if ($null -eq $sess) { $se.Stop(); AddSub $sub "Security Registration Abuse" "WARN" "Cannot initialize WebRequestSession." ([int]$se.ElapsedMilliseconds) }
            else {
                $g = Req "$baseUrl/register" "GET" $sess $null 3
                if (-not $g.ok) { $se.Stop(); AddSub $sub "Security Registration Abuse" "WARN" ("GET /register failed: " + $g.error) ([int]$se.ElapsedMilliseconds) }
                else {
                    $tok = ""; try { $m = [regex]::Match(("" + $g.content), 'name\s*=\s*"[_]?token"\s+value\s*=\s*"([^"]+)"', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase); if ($m.Success) { $tok = ("" + $m.Groups[1].Value) } } catch { $tok = "" }
                    if ($tok -eq "") { $se.Stop(); AddSub $sub "Security Registration Abuse" "WARN" "GET /register did not expose _token." ([int]$se.ElapsedMilliseconds) }
                    else {
                        ...
                        $se.Stop()
                        if ($hit419) { AddSub $sub "Security Registration Abuse" "WARN" "Probe invalid: POST /register returned 419 (csrf_419)." ([int]$se.ElapsedMilliseconds) }
                        elseif ($hit429 -or $kw) { AddSub $sub "Security Registration Abuse" "OK" "Protection signal detected (429/lockout keyword)." ([int]$se.ElapsedMilliseconds) }
                        else { AddSub $sub "Security Registration Abuse" "WARN" "No abuse-protection signal within 6 attempts." ([int]$se.ElapsedMilliseconds) }
                    }
                }
            }
        }
    ...
    $sw.Stop()
    return & $new -Id "security_abuse" -Title "X) Security / Abuse Protection" -Status $status -Summary ("Block status: " + $block) -Details $details -Data $data -DurationMs ([int]$sw.ElapsedMilliseconds)
}
